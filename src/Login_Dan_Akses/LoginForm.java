...
import Login_Dan_Akses.Kurir;
import Login_Dan_Akses.Mitra;
import Login_Dan_Akses.Admin;
import javafx.application.Application;
import javafx.scene.Scene;
import javafx.scene.control.*;
import javafx.scene.layout.*;
import javafx.stage.Stage;
import Role_Admin.DashboardAdmin;
import Role_Kurir.DashboardKurir;
import Role_Mitra.DashboardMitra;
import Role_Mitra.FormDaftarMitra;
import Database.UserDAO;
import Antarmuka_Aset_Visual.GambarLogin;
...
public class LoginForm extends Application {
    ...
    private void login(Stage stage) {
        ...
        if (username.isEmpty() || password.isEmpty()) {
            ErrorHandler.tampilkanPeringatan("Input Kosong", "Username dan Password tidak boleh kosong!");
            return;
        }
        ...
        if (admin.login()) {
            ...
            try {
                if(UserDAO.cekStatusPending(username, password)) {
            ...
            String pesanErrorFinal = "Login gagal! Username atau Password salah.";
            if (mitra.getPesanError() != null && mitra.getPesanError().contains("dinonaktifkan")) {
                pesanErrorFinal = mitra.getPesanError();
            }

            alert.setAlertType(Alert.AlertType.ERROR);
            alert.setContentText(pesanErrorFinal);
            alert.showAndWait();
        }
    }
