...
public class HalamanRiwayatPesanan {
    ...
    private void refreshData() {
        ...
        try {
            List<Pesanan> riwayat = PesananDAO.getRiwayatPesananMitra(idTokoMitra);
            if (riwayat.isEmpty()) {
                boxRiwayat.getChildren().add(new Label("Belum ada riwayat pesanan."));
                return;
            }
            for (Pesanan rwy : riwayat) {
            boxRiwayat.getChildren().add(buatCardPesanan(
                String.valueOf(rwy.getId()), 
                rwy.getTanggal(), 
                String.valueOf((long) rwy.getTotal()), 
                rwy.getStatus()
            ));
        }
    } catch (Exception e) { 
    ...
    private HBox buatCardPesanan(String id, String tanggal, String total, String status) {
        ...
        btnTerima.setOnAction(e -> {
            try {
                PesananDAO.konfirmasiMitraSelesai(id);
                refreshData();
